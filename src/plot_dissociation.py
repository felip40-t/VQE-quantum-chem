...
DATA_DIR = Path(__file__).parent.parent / "results" / "h2" / "data"
PLOTS_DIR = Path(__file__).parent.parent / "results" / "h2" / "plots"
...
def plot_dissociation_curve(
    ...
    out_path = PLOTS_DIR / name.replace('data', 'plot').replace('.csv', '.pdf')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot a VQE dissociation curve from a saved CSV file.")
    parser.add_argument('--csv', type=str, required=True, help='CSV file in results/h2/data/ to plot')
    ...
    fci_path = DATA_DIR / "fci_energies.csv"
    if not fci_path.exists():
        raise FileNotFoundError(f"No FCI reference found at {fci_path}")

    data = pd.read_csv(csv_path)
    fci_data = pd.read_csv(fci_path)
    data = data.merge(fci_data, on='bond_length')
